...
const services = [
  ...
  },
  {
    ...
    const SingleService = ({ title, description }: { title: string; description: string }) => (
      <div className="relative w-[90%] py-10 flex flex-col h-full pointer-events-none">
        <div className="relative w-full mx-1.5 sm:mx-6 p-6 shadow-lg border bg-white pointer-events-auto border-gray-200/40 hover:shadow-black/20 hover:-translate-y-2 transition-all duration-200 flex-1 flex flex-col">
  ...
  return (
    <div className="bodyelement relative w-full h-auto bg-white">
      {/* Hero Section */}
      <div className="relative w-full md:h-160 h-140 flex items-center justify-center bg-[url('/servicesimg1.jpg')] bg-cover bg-center">
      ...
      {/* Services Section */}
      <div className="relative w-full px-4 md:px-8 lg:px-16 h-auto bg-black/2">
        <h1 className="text-black mt-10 p-2 underline decoration-red-600 text-3xl font-libre font-medium">
          Services
        </h1>
